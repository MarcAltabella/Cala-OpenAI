...
export function appendRunEvent(input: Omit<RunEvent, 'id' | 'createdAt'>): RunEvent {
  const event: RunEvent = {
    ...
    input: input.input ? (redact(input.input) as Record<string, unknown>) : undefined,
    output: input.output ? (redact(input.output) as Record<string, unknown>) : undefined,
  };
...
export async function appendRunEventPersisted(input: Omit<RunEvent, 'id' | 'createdAt'>): Promise<RunEvent> {
  ...
  const [row] = await db
    .insert(runEvents)
    .values({ runId: input.runId, phase: input.phase, kind: input.kind, tool: input.tool, input: safeInput, output: safeOutput, summary: input.summary })
    .returning();
  return fromRow(row);
}
