...
describe('runs API', () => { it('queues a delta run without blocking', async () => { const response = await request(app).post('/runs').send({ mode: 'delta' }); expect(response.status).toBe(202); expect(response.body).toMatchObject({ status: 'queued' }); const status = await request(app).get(`/runs/${response.body.id}`); expect(status.body).toMatchObject({ status: 'queued', mode: 'delta' }); }); });
