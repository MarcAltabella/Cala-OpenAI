...
import { listCompanies, listCompanyRuns, type Company, type Run } from '../lib/api';
...
export function CompanyPage({ companyId, onBack }: { companyId: string; onBack: () => void }) {
  ...
  useEffect(() => {
    listCompanies().then((all) => setCompany(all.find((item) => item.id === companyId)));
    listCompanyRuns(companyId).then(setRuns);
  }, [companyId]);
